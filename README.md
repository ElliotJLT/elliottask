...
**Where the AI actually helped, and where I did the work.** I used Claude Opus in Claude Code for the build, running in Conductor so several workstreams could go at once. It is fast at things with a right answer: scaffolding the Next.js 16 app, the deterministic graph layout maths, wiring the external store so transcripts survive a reload, the citation renderer. It is unreliable at taste and at knowing when to stop, so those stayed mine. The product decisions, the audit, the design system, the call on what the five complaints really mean, the repeated no to things that looked plausible and served nothing: that is the part that doesn't delegate. Most of the iteration in the back half of the history is me rejecting output, and the commit messages say why rather than what.
